...
def table_to_pickle(in_table, out_pickle_filename):
    prop_dict={}
    row_dict=csv.DictReader(open(in_table),delimiter="\t")
    iteration=0
    for rows in row_dict:
        if iteration == 0 :
            iteration += 1
            continue
        mon_logger.info("row=" + str(rows))
        row_dict={}
        prop_Key = ''
        for dict_key in list(rows.keys()):
            
            mon_logger.info("Property value " + dict_key + "has the value" + rows[dict_key])
            if dict_key == 'Key':
                prop_Key = rows["Key"]
                mon_logger.info("The Key for this row is:" + prop_Key)
            else:
                row_dict[dict_key] = rows[dict_key] 
