...
class pygoogle:
    ...
    def __init__(self,query,my_logger, pages=10,hl='en',log_level=logging.INFO):
            ...
            if print_results:
                if data.has_key('responseData') and data['responseData'].has_key('results'):
                    for result in  data['responseData']['results']:
                        if result:
                            self.logger.info( '[%s]'%(urllib.unquote(result['titleNoFormatting'])))
                            self.logger.info( result['content'].strip("<b>...</b>").replace("<b>",'').replace("</b>",'').replace("&#39;","'").strip())
                            self.logger.info(urllib.unquote(result['unescapedUrl'])+'\n')                
                else:
